...
var sum = function(array) {
  if (!array.length) {
    ...
    if (array.length > 0) return array[0] + sum(array.slice(1));
  }
};
...
var range = function(x, y) {
  if (y < x) {
    var n = x - 1;
    return n > y ? [n].concat(range(x - 1, y)) : [];
  }
  var n = x + 1;
  return n < y ? [n].concat(range(x + 1, y)) : [];
};
...
var divide = function(x, y) {
  ...
  let result = x < y ? 0 : y - x === y ? 0 : 1 + divide(x - y, y);
...
var minimizeZeroes = function(array) {
  ...
  if (array.slice(1)[0] === 0 && array[0] === 0) {
...
var numToText = function(str) {
  ...
  if (!isNaN(parseInt(firstWord))) {
    output += `${numsToWords[firstWord]}`;
  } else {
    output += `${firstWord}`;
  }
  return output + space + numToText(words.join(' '));
};
